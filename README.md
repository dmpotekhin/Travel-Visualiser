...
- **Routing Service (v4)** — расчёт маршрутов вынесен в заменяемый сервис с
  цепочкой провайдеров **HERE → OSRM → GraphHopper → great-circle** (порядок
  настраивается, strict mode опционален). Фронтенд не знает, кто считал
  маршрут; каждый сегмент GeoJSON помечается провайдером (`provider`).
- **Узоры линий по транспорту (v4)** — авто/автобус/паром — почти сплошная,
  поезд — пунктир, пешком — точки, велосипед — штрих-пунктир, самолёт —
  длинные штрихи поверх дуги большого круга; провайдер показан в карточке
  сегмента и в сводке маршрута.
- **API для интеграций (v4)** — `POST /api/routes` (расчёт сегментов по
  координатам без сохранения) и `GET /api/providers` (диагностика цепочки).
